    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3">
          <div
            className="size-10 lg:size-12 rounded-full bg-linear-to-br bg-primary overflow-hidden cursor-pointer hover:opacity-80 transition-opacity"
        ...
        <div className="mb-3">
          ...
          {isLarge && (
            <span className="text-secondary ml-1 hover:underline">
              See more
            </span>
          )}
        ...
        <div className="flex items-center gap-4 text-sm mb-3 text-muted-foreground">
          {postData.viewCount > 0 && (
            <div className="flex items-center gap-1">
              <span className="font-medium text-foreground">
                {postData.viewCount.toLocaleString()}
              </span>
              <span>Views</span>
            </div>
          )}
          <div className="flex items-center gap-1">
            ...
            <span className="font-medium text-foreground">
              {postData.commentCount.toLocaleString()}
            </span>
          </div>
          <div className="flex items-center gap-1">
            <Repeat2 size={16} />
            <span className="font-medium text-foreground">
              {postData.shareCount.toLocaleString()}
            </span>
          </div>
        </div>

        <PostReactionActions postId={postData.id} userId={postData.author.id} />
      </div>
    </article>
  );
}
