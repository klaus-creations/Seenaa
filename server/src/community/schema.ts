...
export const communityMember = pgTable(
  'community_member',
  {
    ...
    // member = empty, admins = fixed some, creator = all
  ...
  (table) => ({
    uniqueCommunityUser: uniqueIndex('unique_community_user').on(table.communityId, table.userId),
    idxCommunityStatus: index('idx_comm_status').on(table.communityId, table.status),
  }),
);
